...
(spawn (assert (advertise (observe (websocket-message any-client tcp-server-id _))))
       (on (asserted (advertise (websocket-message ($ c any-client) tcp-server-id _)))
           (handle-connection tcp-server-id c)))
(spawn (assert (advertise (observe (websocket-message any-client ssl-server-id _))))
...
(define (handle-connection s c)
  (spawn (stop-when (retracted (advertise (websocket-message c s _))))
